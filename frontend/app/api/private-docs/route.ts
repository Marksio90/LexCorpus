...
export async function POST(req: NextRequest) {
  ...
  if (!["pro", "kancelaria"].includes((session.user as { tier?: string }).tier ?? "")) {
  ...
  // Zapisz plik na dysk współdzielony (volume: data/private)
  ...
  const filePath = join(UPLOAD_DIR, `${doc.id}_${safeFilename}`);
  await writeFile(filePath, Buffer.from(await file.arrayBuffer()));

  // Kolejkuj przetwarzanie przez Celery worker (zamiast spawn subprocess)
  try {
    const enqueueRes = await fetch(`${BACKEND}/internal/enqueue-document`, {
      method: "POST",
      headers: {
    ...
    if (!enqueueRes.ok) {
      const detail = await enqueueRes.text();
      throw new Error(`Błąd kolejkowania: ${detail}`);
    }
  } catch (err) {
    // Oznacz dokument jako błąd jeśli nie udało się zakolejkować
    await prisma.privateDocument.update({
      where: { id: doc.id },
